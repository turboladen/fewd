 ...
 *
 * This is the implementation module: it carries the `react-markdown` import (and
 * its heavy parser tree). It is loaded on demand via `React.lazy` from the
 * `RecipeMarkdown` wrapper, so the markdown machinery splits out of the main
 * bundle and only loads when a recipe/notes/cook view actually renders.
 */
...
export default function RecipeMarkdownImpl({ markdown, variant = 'detail' }: Props) {
