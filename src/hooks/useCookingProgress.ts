 ...
 * `totalSteps` bounds the current-step search.
 *
 * Progress is keyed by `recipeId`; switching recipes loads that recipe's saved
 * state. {@link CookingProgressState.reset} clears persistence and is wired to
 * explicit cooking-mode exit (a reload, which does not call reset, restores).
 */
export function useCookingProgress(recipeId: string, totalSteps: number): CookingProgressState {
  // One read on first mount — a single storage snapshot seeds both sets so they
  // can never hydrate from inconsistent reads.
  const [initial] = useState(() => loadCookingProgress(recipeId))
  ...
  // state — and the lazy initializer already covers the first mount.
  const [loadedRecipeId, setLoadedRecipeId] = useState(recipeId)
  if (loadedRecipeId !== recipeId) {
    const saved = loadCookingProgress(recipeId)
  ...
  }, [recipeId, completedSteps, addedIngredients])
