...
- `--all` replaces the scopes when the classifier fails, prints nothing, or prints a line that is not a known scope. The classifier prints `none` for an empty diff and `all` when it cannot diff against the base.
