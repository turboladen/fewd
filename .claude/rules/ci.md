...
`just ci` is what the Claude Code `PreToolUse` hook (`.claude/hooks/ci-before-push.sh`, registered in `.claude/settings.json`) runs before any command containing `git push` or `gh pr create`, with a 300s timeout. It is not a git hook. It runs in the session's current working directory, so push from the worktree whose branch you are pushing; `SKIP_CI_HOOK=1` bypasses it for one command.
