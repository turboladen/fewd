...
- Default verification for ordinary route, service, and handler changes is `cargo test`; the release-build requirement for schema and feature changes is in the migrations rule.

## Formatting and lint

```bash
cargo fmt --all                                                         # whole workspace
cd server && cargo clippy --all-targets --all-features -- -D warnings  # what CI runs
```

Use `cargo fmt --all`: bare `cargo fmt` formats only the current directory's crate and misses `server/migration`.
