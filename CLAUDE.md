...
- A Claude Code hook gates any Bash command containing `git push` or `gh pr create` and blocks it on failure. It checks the tree the command targets, so `cd .claude/worktrees/X && git push` checks X. Judged by the paths the outgoing commits and the working tree change, it runs nothing when there are none, `dprint check` and `typos` when only markdown, `LICENSE` or `.beads/` files change, and `just ci` otherwise. It also blocks a command whose published ref it cannot check, such as a push of another branch or one that follows a `git switch`. `gh stack submit` and `gh stack sync` push without it, so run `just ci` or /verify before those.
