...
- **The JSONL is a full DB snapshot**, not a per-PR diff. `bd`'s pre-commit hook re-exports every issue and memory row on every commit. So issues that show up in a PR's `.beads/issues.jsonl` diff are _not_ necessarily "added in this PR" — they could have been created in a different terminal session, on main, or on another branch, and just got captured by this branch's next export. Issue rows for beads unrelated to the PR's bead ID are normal ride-alongs, not unrelated-changes-in-this-PR. Removing them would require manually editing the JSONL outside `bd`'s control, which corrupts the snapshot. Cross-reference: this exact concern was flagged repeatedly on PRs #35, #37, #39, #43, #44 in the fewd-2y6 / fewd-b3x series; the answer has been the same each time.
- **Memory rows reorder non-deterministically.** Rows tagged `"_type":"memory"` are stored in a `HashMap` and `bd export` walks them in iteration order. The order changes between commits even when no content changes. Don't suggest "sort the memory rows" or "this looks like noise to clean up" — reverting requires manual JSONL editing (the anti-pattern above) and the rows reshuffle on the next commit anyway.
