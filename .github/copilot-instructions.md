...
2. **`.claude/rules/`** — `cross-boundary.md` lists invariants the type system doesn't enforce (Title-Case `meal_type`, slot-encoded `order_index`, DB path resolution), and `mcp.md` summarizes the tool-design principles. `CLAUDE.md` holds the always-relevant essentials.
...
`server/src/mcp/` follows seven design principles summarized in `.claude/rules/mcp.md`. Two come up in review most often:
...
`.beads/issues.jsonl` is **not tracked by git** in this repo. The Dolt DB under `.beads/` is the source of truth; `bd export` writes the JSONL as a local mirror only, so it never appears in a PR diff. A few remaining conventions that look editable from a diff but aren't:
...
- **Closure happens after the PR merges**, on `main`, via `bd close <id>` + `bd dolt push`. It produces no commit, because the JSONL mirror is untracked. Marking a bead closed while its PR is still in review would have `bd list --status=open` falsely report the fix is shipped.
