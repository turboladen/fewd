...
- **Memory rows do not belong in the JSONL.** As of bd 1.0.4+, `bd export` excludes `bd remember` memories by default, so the committed `.beads/issues.jsonl` carries issue rows only — memories live in the Dolt DB (source of truth), synced via `bd dolt push`. A `{"_type":"memory",...}` row in a diff is a stale snapshot from before this convention, not something to preserve; the fix is a bare `bd export -o .beads/issues.jsonl`, not hand-editing. (Pre-1.0.4 the JSONL did carry memory rows, stored in a `HashMap` and reordered non-deterministically on every commit; that "is this cleanup?" review noise — flagged on PRs #35/#37/#39 — is gone now. Decided in fewd-040.)
