...
- **The JSONL is a full issue-table snapshot**, not a per-PR diff. `bd`'s pre-commit hook re-exports every issue row on every commit. So issues that show up in a PR's `.beads/issues.jsonl` diff are _not_ necessarily "added in this PR" — they could have been created in a different terminal session, on main, or on another branch, and just got captured by this branch's next export. Issue rows for beads unrelated to the PR's bead ID are normal ride-alongs, not unrelated-changes-in-this-PR. Removing them would require manually editing the JSONL outside `bd`'s control, which corrupts the snapshot. Cross-reference: this exact concern was flagged repeatedly on PRs #35, #37, #39, #43, #44 in the fewd-2y6 / fewd-b3x series; the answer has been the same each time.
- **Memory rows do not belong in the JSONL.** As of bd 1.0.4+, `bd export` excludes `bd remember` memories by default, so the committed `.beads/issues.jsonl` carries issue rows only — memories live in the Dolt DB (source of truth), synced via `bd dolt push`/`pull`. A `{"_type":"memory",...}` row in a diff is a stale snapshot from before this convention, not something to preserve; the fix is a bare `bd export -o .beads/issues.jsonl`, not hand-editing. (Pre-1.0.4 the JSONL did carry memory rows, stored in a `HashMap` and reordered non-deterministically on every commit; that "is this cleanup?" review noise — flagged on PRs #35/#37/#39 — is gone now. Decided in fewd-040.)
