...
impl FewdMcp {
    ...
    #[tool(
        name = "update_recipe",
        description = "Revise an existing recipe when the user corrects or improves one already in the catalog — call `get_recipe` FIRST for the `slug` and current values (use `create_recipe` when the dish isn't in the catalog, or `adapt_recipe` to save a changed copy and keep the original). Returns the full updated recipe. Only the fields you send are written; omitted, null, or blank-string fields are left unchanged. List and structured fields REPLACE the stored value whole: a partial `ingredients` array deletes every ingredient you left out, so send the complete list back. Renaming does not change the slug. Changing `servings` does not rescale `ingredients`, so send both to resize. An invalid field, such as a time `unit` other than minutes, hours, or days, rejects the whole call and writes nothing. Not writable here: `is_favorite` (call `favorite_recipe`), `rating` (call `rate_recipe`, or `unrate_recipe` to clear it), `source`, `source_url`, the parent recipe, and the slug. Example: {\"slug\":\"beef-taco-bowls\",\"notes\":\"double the chili powder\"}",
