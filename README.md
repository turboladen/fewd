...
| `favorite_recipe(slug, is_favorite)`                   | Mark a recipe as a family favorite, or unmark it. `is_favorite` is set absolutely rather than toggled, so repeating a call changes nothing. Drives the `list_curated_recipes` shortlist and `search_recipes`'s `is_favorite` filter                                                                |
